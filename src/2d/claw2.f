...
c
      ...
      dtmax = dt
      nv(2) = 0
      !maux = method(7)    ! Now brought in from arg list
c
...
c
      nqwork = (mx + 2*mbc) * (my + 2*mbc) * meqn  !# size of q array
      if (method(5).lt.2) then
...
c        # extend data from grid to bordering boundary cells:
         call bc2(meqn,mbc,mx,my,xlower,ylower,
...
c            # subroutine step2.
             call copyq2(meqn,mbc,mx,my,q,work(i0qwrk2))
c
c            # source terms over a half time step:
             dt2 = dt / 2.d0
             call src2(meqn,mbc,mx,my,xlower,ylower,
...
c        # taking the source term step.
         call copyq2(meqn,mbc,mx,my,q,work(i0qwrk1))
c
...
c
            call dimsp2(maxm,meqn,mwaves,maux,mbc,mx,my,
     &                  work(i0qwrk1),q,aux,
...
c             # solution is stored in all cases method(5) = 0,1, or 2. 
              t = told
              call copyq2(meqn,mbc,mx,my,work(i0qwrk2),q)
c
              if (method(4).eq.1) then
c                # verbose mode
                 write(6,602)
