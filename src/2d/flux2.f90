...
!     =====================================================
    ...
    amdq,apdq,cqxx,bmasdq,bpasdq,rpn2,rpt2,use_fwave,abl,i_abl_lower,i_abl_upper)
!     =====================================================
...
!f2py intent(callback) rpn2,rpt2
    ...
    dimension abl(1-num_ghost:maxm+num_ghost)
    dimension dtdxave(1-num_ghost:mx+num_ghost)
...
!     -----------------------------
    ...
    ! Store dtdxave for second order corrections so that it may be scaled for
    ! ABL if needed
    do i = 2-num_ghost, mx+num_ghost
      dtdxave(i) = 0.5d0 * (dtdx1d(i-1) + dtdx1d(i))
    end do

    ! apply ABL if needed
    if (abl_type /= 0) then
        do i = 2-num_ghost, i_abl_lower
            dtdxave(i) = dtdxave(i) * abl(i)
        end do
...
!     # Set qadd for the donor-cell upwind method (Godunov)
    do i = 1, mx+1
        do m = 1, num_eqn    ! qadd(:,i-1) is still in cache from last cycle of outer loop
            qadd(m,i-1) = qadd(m,i-1) - amdq(m,i)
        end do
        do m = 1, num_eqn
            qadd(m,i) = qadd(m,i) - apdq(m,i)
        end do
    end do
...
!     # Second order corrections:
    if (use_fwave.eqv. .FALSE. ) then
        ...
        !            # modified in Version 4.3 to use average only in cqxx, not transverse
            ...
            do mw=1,num_waves    ! Traverse the wave array in memory-contiguous fashion
                do m=1,num_eqn
                    ...
                    * (1.d0 - dabs(s(mw,i))*dtdxave(i)) * wave(m,mw,i)
                enddo
            enddo
            do m = 1, num_eqn
        ...
        if (abl_type /= 0) then
            print *, "ABL not implemented yet for F-wave splitting"
            stop
        end if
        do i = 2-num_ghost, mx+num_ghost
            do m = 1, num_eqn
